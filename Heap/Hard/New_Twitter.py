...
import heapq
def __init__(self):
    self.tweets = {} #userId -> list of (timeStamp,tweetId) pairs
    self.following = {} #userId -> set followeeeIds
    self.timestamp = 0 
...
def getNewsFeed(self,userId):
    ...
    for followeeId in self.following[userId]:
        if followeeId in self.tweets :
            for timestamp,tweetId in reversed(self.tweets[followeeId][-10:]):
                heapq.heappush(heap,(-timestamp,tweetId))
    
    newsFeed = []
    while heap and len(newsFeed) < 10 :
        newsFeed.append(heapq.heappop(heap)[1])
