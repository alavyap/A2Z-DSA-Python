...
def vOrder(root):
    ...
    def preorder(node, row,col):
        ...
        position[col].append((row,node.val))
    ...
    for col in sorted(position.keys()):
        colum_sorted = sorted(position[col], key = lambda x : (x[0],x[1]))
        
        result.append([val for row, val in colum_sorted])
